...
class TemplateWorkoutStore internal constructor(
    ...
    suspend fun addSet(
        ...
        try {
            ...
            dao.touchWorkout(workoutId)
            val exerciseRef = fs.templateRef(workoutId)
    ...
    suspend fun updateSet(
        ...
        try {
            ...
            fs.resolveSetDocument(exerciseRef, setId).set(
                mapOf(
                    "setId" to setId,
    ...
    suspend fun deleteSelectedExercise(exerciseId: String, workoutId: String) {
        ...
        try {
            val exerciseDocRef = fs.templateRef(workoutId)
                .collection(FirestorePaths.EXERCISES)
                .document(exerciseId)
            val batch = firestore.batch()
    ...
    suspend fun syncMyWorkouts(userId: String) {
        ...
        try {
            ...
            for (workout in remoteWorkouts) {
                val exerciseSnap = fs.templateRef(workout.workoutId)
                    .collection(FirestorePaths.EXERCISES)
                    .get()
                    .await()
                ...
                for (ex in remoteExercises) {
                    ...
                    val setsSnap = fs.loadSetDocuments(
                        fs.templateRef(workout.workoutId)
                            .collection(FirestorePaths.EXERCISES)
    ...
    suspend fun getMaxOfExerciseSet(exerciseId: String): Int? {
        return dao.getMaxOfExerciseSet(exerciseId)
    }
}
