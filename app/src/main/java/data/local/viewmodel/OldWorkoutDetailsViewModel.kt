...
class OldWorkoutDetailsViewModel(
    ...
    fun deleteHistorcialWorkoutById() {
        viewModelScope.launch {
            repo.sessions.deleteHistoricalWorkoutById(_sessionId.value)
        }
    }
    ...
    fun enterEditMode(current: WorkoutHistoryFull) {
        ...
        original = ordered
        _draft.value = ordered.copy(
            exerciseWithSets = ordered.exerciseWithSets.map { ex ->
        ...
        viewModelScope.launch {
            ...
            draft.exerciseWithSets.forEachIndexed { index, ex ->
                if (ex.exerciseLog.logId < 0) {
                    val newId = repo.sessions.addExerciseLog(
                    ...
                    val oldIndex = before.exerciseWithSets
            ...
            val toUpdate = draftSets.filter { set ->
            ...
            toUpdate.forEach {
                repo.sessions.addSetLog(
                    it.logOwnerId, it.setId, it.reps, it.weight, it.setIndex, it.clicked
                )
            }
            toInsert.forEach { set ->
                ...
                repo.sessions.addSetLog(
                    ownerId, 0L, set.reps, set.weight, set.setIndex, set.clicked
                )
            }
            ...
            exerciseWithSets = current.exerciseWithSets.map { exercise ->
                exercise.copy(
                    setLogs = exercise.setLogs.map { set ->
                        if (set.setId == setId) set.copy(reps = reps, weight = weight, clicked = true) else set
                    }
                )
            }
        )
    ...
    fun addDraftSet(logId: Long) {
        val current = _draft.value ?: return
        val tempId = current.exerciseWithSets
            .flatMap { it.setLogs }
        ...
        _draft.value = current.copy(
            exerciseWithSets = current.exerciseWithSets.map { exercise ->
                ...
                exercise.copy(
                    setLogs = exercise.setLogs + SetLogEntity(
                        setId = tempId,
                        logOwnerId = logId,
                        reps = last?.reps ?: 0,
        ...
        _draft.value = current.copy(
            exerciseWithSets = current.exerciseWithSets.map { exercise ->
                if (exercise.setLogs.none { it.setId == setId }) {
                    ...
                    val remaining = exercise.setLogs
    ...
    fun updateDraftDuration(totalSeconds: Long) {
        val current = _draft.value ?: return
        _draft.value = current.copy(
            workoutHistory = current.workoutHistory.copy(
    ...
    fun addDraftExercisesFromCatalog(catalogExercises: List<ExerciseCatalogEntity>) {
        ...
        val current = _draft.value ?: return
        val existingNames = current.exerciseWithSets
            .map { it.exerciseLog.exerciseName.lowercase() }
            .toSet()

        var nextLogId = current.exerciseWithSets
            .minOfOrNull { it.exerciseLog.logId }
            ?.let { if (it < 0) it - 1 else -1L }
            ?: -1L
        var nextSetId = current.exerciseWithSets
            .flatMap { it.setLogs }
            .minOfOrNull { it.setId }
            ?.let { if (it < 0) it - 1 else -1L }
            ?: -1L
        ...
        val additions = catalogExercises.mapNotNull { catalog ->
    ...
    fun moveDraftExercises(fromIndex: Int, toIndex: Int) {
        ...
        _draft.value = current.copy(
            exerciseWithSets = list.mapIndexed { index, item ->
